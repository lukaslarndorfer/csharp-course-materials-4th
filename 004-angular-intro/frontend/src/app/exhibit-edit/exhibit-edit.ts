...
import {Component, computed, inject, input, numberAttribute, signal, Signal, WritableSignal} from '@angular/core';
...
export class ExhibitEdit {
  ...
  public async handleFormSubmit(): Promise<void> {
    await submit(this.exhibitForm, async form => {
      ...
      const [presentVerb, pastVerb] = this.editMode() ? ["update", "updated"] : ["add", "added"];
